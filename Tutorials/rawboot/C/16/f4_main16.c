...
void printGDTEntry(GDT* gdt, size16_t index) {
	...
	printf("SegmentLimit: 0x%01X%04X\n",
	       getSegmentLimitHigh4(gdt, index),
	       getSegmentLimitLow16(gdt, index));

	printf("BaseAddress: 0x%04X%04X\n",
	       getBaseAddressHigh16(gdt, index),
	       getBaseAddressLow16(gdt, index));
