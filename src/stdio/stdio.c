...
static inline enum PRINTF_TYPE char_to_printf_tag(char c)
{
	switch (c)
	{
	case 'c': return PRINTF_TAG_CHAR;
	case 's': return PRINTF_TAG_STRING;
	case 'd': return PRINTF_TAG_INT;
	case 'u': return PRINTF_TAG_UINT32_T;
	case 'f': return PRINTF_TAG_FLOAT;
	case 'h': return PRINTF_TAG_HEX;
	case 'b': return PRINTF_TAG_BINARY;
	default: return PRINTF_TAG_INVALID;
	}
}
...
const char *printf_fmt_option_to_str(enum PRINTF_FMT_OPTION opt)
{
	switch (opt)
	{
	case FMT_OPTION_NONE: return "FMT_OPTION_NONE";
	case FMT_OPTION_NOT_IMPLEMENTED: return "FMT_OPTION_NOT_IMPLEMENTED";
	case FMT_OPTION_COULD_NOT_CONVERT: return "FMT_OPTION_COULD_NOT_CONVERT";
	case FMT_OPTION_PAD: return "FMT_OPTION_PAD";
	case FMT_OPTION_PRECISION: return "FMT_OPTION_PRECISION";
	default: return "FMT_DEFAULT";
	}
}
...
const char *printf_type_to_str(enum PRINTF_TYPE tag)
{
	switch (tag)
	{
	case PRINTF_TAG_CHAR: return "PRINTF_TAG_CHAR";
	case PRINTF_TAG_STRING: return "PRINTF_TAG_STRING";
	case PRINTF_TAG_INT: return "PRINTF_TAG_INT";
	case PRINTF_TAG_UINT32_T: return "PRINTF_TAG_UINT32_T";
	case PRINTF_TAG_FLOAT: return "PRINTF_TAG_FLOAT";
	case PRINTF_TAG_HEX: return "PRINTF_TAG_HEX";
	case PRINTF_TAG_BINARY: return "PRINTF_TAG_BINARY";
	case PRINTF_TAG_INVALID: return "PRINTF_TAG_INVALID";
	default: return "PRINTF_TAG_UNKNOWN";
	}
}
...
void vkprintf(const char *fmt, va_list args)
{
	...
	for (uint8_t i = 0; i < len; i++)
	{
		...
		case PRINTF_TAG_BINARY:
		{
			uint32_t					   v	= va_arg(args, uint32_t);
			struct PRINTF_FIELD_PROPERTIES info = printf_information[i];
			print_binary_var_no_newline(v, info.option == FMT_OPTION_PAD ? info.option_num : 0);
			break;
		}
		default: break;
		}
