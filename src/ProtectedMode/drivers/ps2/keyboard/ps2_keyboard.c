...
void set_active_keyboard_for_commands(uint8_t keyboard_idx)
{
	assert(kps.number_of_keyboards > 1,
		"Makes no sense to change the active keyboard when there's just 1!\nThis was wrongly called with: keyboard_idx = %u, "
		"_number_of_keyboard = %u\n",
...
bool validate_keyboard_command(uint8_t cmd)
{
	...
	case KCB_reset_and_self_test: return true;
...
const char *ps2_keyboard_error_to_string(enum ps2_keyboard_error_code code)
{
	switch (code)
	{
	case PS2_KB_ERR_none: return "No error";
	case PS2_KB_ERR_invalid_command: return "Invalid command (Runtime check in static inline wrapper)";
	case PS2_KB_ERR_could_not_send_command: return "Could not send command";
	case PS2_KB_ERR_could_not_recieve_response: return "Could not receive response";
	case PS2_KB_ERR_resend_too_much: return "Resend attempted too many times";
	case PS2_KB_ERR_non_ack_response: return "Non-acknowledged response";
	case PS2_KB_ERR_response_was_not_echo: return "Response was not echo";
	case PS2_KB_ERR_invalid_scancode: return "Invalid scancode";
	default: return "Unknown error";
	}
}

/* ================================ The main command ================================= */
static inline enum ps2_controller_error_code _send_command_or_data_to_ps2_keyboard(enum KeyboardCommandByte command)
{
...
void test_ps2_keyboard_commands()
{
	...
	if (scan_code_set.err)
	{
		kprintf("Error getting the scan code set back. Error: %d, Error Name: |%s|\n", scan_code_set.err,
			ps2_keyboard_error_to_string(scan_code_set.err));
	}
