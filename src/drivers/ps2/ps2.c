...
enum handle_ps2_setup_status handle_ps2_setup_one_keyboard_one_mouse(struct ps2_initialize_device_state device_sates)
{
	...
	if (port1_super_type == PS2_DST_unknown && port2_super_type == PS2_DST_unknown)
	{
		abort_msg("This should be logically impossible, idrember the code. Crash here!\n");
	}

	if (port1_super_type == PS2_DST_unknown)
	{
		// Handle port 2 only. Then return the error

		if (port2_super_type == PS2_DST_keyboard)
		{
		}
		else if (port2_super_type == PS2_DST_mouse)
		{
		}
	...
	if (port2_super_type == PS2_DST_unknown)
	{
		...
		// Handle port 1 only. Then return the error
	...
	return PS2_HS_ERR_one_keyboard_one_mouse;
}
