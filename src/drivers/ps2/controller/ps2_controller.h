...
extern "C"
{
	...
	STATIC_ASSERT(sizeof(PS2_StatusRegister_t) == 1, "ps2_satus_register_t must be of size: 1 byte");
	...
	STATIC_ASSERT(sizeof(ps2_status_register_uts_t) == 1, "ps2_satus_register_t must be of size: 1 byte");
	...
	typedef enum PS2_CommandByte
	{
		...
		PS2_CB_test_second_ps2_port	   = 0xA9, // response byte type: (passed = 0x00, 0x01 = clock line stuck low, 0x02 = clock line stuck
											   // high, 0x03 = data line stuck low, 0x04 = data line stuck high)
		...
		PS2_CB_pulse_output_line_low_end =
			0xFF, // response byte type: None
	...
	STATIC_ASSERT(sizeof(PS2_CommandByte_t) == 4, "PS2_ConfigurationByte_t is stupidly 4 bytes because enums ALWAYS use ints. BAD C!");
	...
	*/
	...
	STATIC_ASSERT(sizeof(ps2_command_byte_t) == 4, "The union's size is stupidly 4 bytes because enums ALWAYS use ints. BAD C!");
	/* ==================================================================================================================================================*/
	...
	STATIC_ASSERT(sizeof(PS2_ConfigurationByte_t) == 1, "PS2_ConfigurationByte_t must be of size: 1 byte");
	...
	STATIC_ASSERT(sizeof(ps2_configuration_byte_uts_t) == 1, "ps2_configuration_byte_uts_t must be of size: 1 byte");
	...
	static inline const char *PS2_TestPortResponse_to_string(enum PS2_TestPortResponse r)
	{
		switch (r)
		{
		case PS2_TPR_passed: return "PS2_TPR_passed";					  // 0x00
		case PS2_TPR_clock_stuck_low: return "PS2_TPR_clock_stuck_low";	  // 0x01
		case PS2_TPR_clock_stuck_high: return "PS2_TPR_clock_stuck_high"; // 0x02
		case PS2_TPR_data_stuck_low: return "PS2_TPR_data_stuck_low";	  // 0x03
		case PS2_TPR_data_stuck_high: return "PS2_TPR_data_stuck_high";	  // 0x04
		default: return "Invalid PS2_TestPortResponse!";
		}
	}
	...
	static inline const char *PS2_TestControllerResponse_to_string(enum PS2_TestControllerResponse r)
	{
		switch (r)
		{
		case PS2_TCR_passed: return "PS2_TCR_passed";
		case PS2_TCR_failed: return "PS2_TCR_failed";
		default: return "Invalid PS2_TestControllerResponse!";
		}
	}
	...
	STATIC_ASSERT(sizeof(PS2_ControllerOutputPort_t) == 1, "PS2_ControllerOutputPort_t must be of size: 1 byte");
	...
	static inline const char *PS2_PortNumber_to_string(enum PS2_PortNumber port_number)
	{
		switch (port_number)
		{
		case PS2_PN_port_one: return "1: PS2_PN_port_one";
		case PS2_PN_port_two: return "2: PS2_PN_port_two";
	...
	*/
	union ___anon1
	{
		...
		__attribute__((unused)) uint8_t ___not_a_command; // Can't and shouldn't even send it! This case should never be inside a tagged
														  // response, an error should have happened.
	...
	*/
	...
	static inline const char *PS2_ResponseType_to_string(enum PS2_ResponseType response_type)
	{
		...
		{
		case PS2_RT_not_a_command: return "Not a command";
		case PS2_RT_none: return "None";
		case PS2_RT_unknown: return "Unknown";
		case PS2_RT_controller_configuration_byte: return "Controller config";
		case PS2_RT_test_port: return "Test port";
		case PS2_RT_test_controller: return "Test controller";
		case PS2_RT_controller_output_port: return "Controller output";
		default: return "Invalid Response Type!";
		}
	}
	...
	*/
	...
	/*
	The PS/2 Controller accepts commands and performs them. These commands should not be confused with bytes sent to a PS/2 device (e.g.
	keyboard, mouse).
	...
	then the response byte needs to be read from IO Port 0x60 after making sure it has arrived (by making sure bit 0 of the Status Register
	is set).
