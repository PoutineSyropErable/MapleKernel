/*
    ...
    Copyright 2014 / the`janitor / < email:
   base64dec(dGhlLmphbml0b3JAcHJvdG9ubWFpbC5jb20=) >
...
static const char *const regs_str[] = {
    ...
    "spl",   "bpl",   "sil",   "dil",   "r8l",   "r9l",   "r10l",  "r11l",
    "r12l",  "r13l",  "r14l",  "r15l",
...
static const char *const opsize_str[] = {"",      "byte",  "word",  "dword",
                                         "fword", "qword", "tword", "oword",
                                         "yword", "zword"};

static const char *const get_opsize_str(uint8_t a) { return opsize_str[a]; }
...
static uint8_t get_uint8(x86_dasm_context_t *x86_dctx, int i) {
    return (i == -1 ? x86_dctx->buffer[x86_dctx->pos++] : x86_dctx->buffer[i]);
}

static uint16_t get_uint16(x86_dasm_context_t *x86_dctx, int i) {
    uint16_t r;

    uint8_t *p =
        (i == -1 ? &x86_dctx->buffer[x86_dctx->pos] : &x86_dctx->buffer[i]);
...
static uint32_t get_uint32(x86_dasm_context_t *x86_dctx, int i) {
    uint32_t r;
...
static uint64_t get_uint64(x86_dasm_context_t *x86_dctx, int i) {
    uint64_t r;
...
static int decode_modrm_mod_rm(x86_dasm_context_t *x86_dctx, x86_operand_t *op,
                               uint8_t base_reg);

/***************************************************************************/

static uint8_t get_modrm(x86_dasm_context_t *x86_dctx) {
    if (!x86_dctx->p_modrm) {
...
static uint8_t get_sib(x86_dasm_context_t *x86_dctx) {
    if (!x86_dctx->p_sib) {
	x86_dctx->p_sib = true;
...
static void check_modrm_mod_rm(x86_dasm_context_t *x86_dctx,
...
static void check_modrm_reg(x86_dasm_context_t *x86_dctx,
                            const x86_table_entry_t *te) {
    /* check all the operands (currently max 4) */
    for (int i = 0; i < 4; i++) {
	switch (te->operands[i].am) {
...
static uint8_t get_osize(x86_dasm_context_t *x86_dctx, uint16_t flags) {
    /*
    ...
    */
	...
	if (flags & PF_F64) {
	    ...
	    if ((REX_W) || (x86_dctx->pfx_p_vex3b && VEX_3B_W) ||
...
static uint8_t get_asize(x86_dasm_context_t *x86_dctx) {
    /*
        D (default) field of the code-segment (CS) is assumed to be 32-bit
        for protected mode / compatibility mode and 16-bit for real mode /
        virtual 8086 mode
...
static int decode_prefixes(x86_dasm_context_t *x86_dctx) {
    int i;
...
decode_prefixes_done:
    ...
    */
    if (b[i] == PREFIX_VEX_3B &&
        (x86_dctx->dmode == X86_DMODE_64BIT || VEX_3B_PM(b[i + 1]))) {
	if (i > (MAX_INST_LENGTH - 5) || x86_dctx->pfx_p_osize ||
	    x86_dctx->pfx_p_rex || x86_dctx->pfx_p_lock ||
	    x86_dctx->pfx_p_rep || x86_dctx->pfx_p_repne) {
	    /*
    ...
    } else if (b[i] == PREFIX_VEX_2B &&
               (x86_dctx->dmode == X86_DMODE_64BIT || VEX_2B_PM(b[i + 1]))) {
	if (i > (MAX_INST_LENGTH - 4) || x86_dctx->pfx_p_osize ||
	    x86_dctx->pfx_p_rex || x86_dctx->pfx_p_lock ||
	    x86_dctx->pfx_p_rep || x86_dctx->pfx_p_repne) {
	    /*
	        any VEX-encoded instruction with a REX, LOCK, 66H, F2H,
    ...
    } else if (b[i] == PREFIX_EVEX &&
               (x86_dctx->dmode == X86_DMODE_64BIT || EVEX_PM(b[i + 1]))) {
	/*
	...
	*/
	...
	if (i > (MAX_INST_LENGTH - 6) || x86_dctx->pfx_p_osize ||
	    x86_dctx->pfx_p_rex || x86_dctx->pfx_p_lock ||
	    x86_dctx->pfx_p_rep || x86_dctx->pfx_p_repne) {
	    /*
	        any VEX-encoded instruction with a REX, LOCK, 66H, F2H,
    ...
    } else if (b[i] == PREFIX_XOP &&
               (x86_dctx->dmode == X86_DMODE_64BIT || XOP_PM(b[i + 1])) &&
               XOP_VALID(b[i + 1])) {
	if (i > (MAX_INST_LENGTH - 5) || x86_dctx->pfx_p_osize ||
	    x86_dctx->pfx_p_rex || x86_dctx->pfx_p_lock ||
	    x86_dctx->pfx_p_rep || x86_dctx->pfx_p_repne) {
    ...
    dbg_msg("\n"
            ...
            "\t pfx_p_osize     : %i\n"
            "\t x86_dctx->pos   : %i\n",
            x86_dctx->pfx_seg, x86_dctx->pfx_rex, x86_dctx->pfx_last,
            x86_dctx->pfx_mandatory, x86_dctx->pfx_vex[0], x86_dctx->pfx_vex[1],
            x86_dctx->pfx_p_osize, x86_dctx->pos);

    return i;
}

static int decode_opcode(x86_dasm_context_t *x86_dctx,
                         const x86_table_entry_t **te_out) {
    int done = false;
    ...
    while (!done) {
	dbg_msg("table: %s, method: %i", link->table_name, link->lookup_method);
    ...
    *te_out = te;
	...
	} else if ((te->properties & PF_VSIB_Y) != 0) {
	    x86_dctx->vsib_base = X86_REG_YMM0;
	} else if ((te->properties & PF_VSIB_Z) != 0) {
...
static int decode_data(x86_dasm_context_t *x86_dctx, x86_operand_t *op,
                       uint8_t type) {
	...
	case X86_ASIZE_16BIT:
	    op->asize = X86_ASIZE_WORD;
	    size = X86_OPSIZE_WORD;
	    break;

	case X86_ASIZE_32BIT:
	    op->asize = X86_ASIZE_DWORD;
	    size = X86_OPSIZE_DWORD;
	    break;

	case X86_ASIZE_64BIT:
	    op->asize = X86_ASIZE_QWORD;
	    size = X86_OPSIZE_QWORD;
	    break;
...
static int decode_gpr(x86_dasm_context_t *x86_dctx, uint8_t opsize, uint8_t reg,
                      uint8_t *result) {
    ...
    *result = X86_REG_NONE;
    ...
    case X86_OPSIZE_WORD:
	*result = (X86_REG_AX + reg);
	break;
    case X86_OPSIZE_DWORD:
	*result = (X86_REG_EAX + reg);
	break;
    case X86_OPSIZE_QWORD:
	*result = (X86_REG_RAX + reg);
	break;
...
static int decode_reg(x86_dasm_context_t *x86_dctx, x86_operand_t *op,
                      uint8_t base_reg, uint8_t reg) {
...
static void decode_sib(x86_dasm_context_t *x86_dctx, x86_operand_t *op) {
    uint8_t i = 0;
	...
	*/
	if (x86_dctx->vsib_base == X86_REG_NONE) {
		...
		i = X86_REG_ZMM0;
	    } else if (op->size == X86_OPSIZE_YWORD) {
		i = X86_REG_YMM0;
	    } else {
		i = X86_REG_XMM0;
	    }
	...
	if (op->mexpr.index == X86_REG_ESP || op->mexpr.index == X86_REG_RSP) {
	    /* no index register is encoded */
	    op->mexpr.index = X86_REG_NONE;
	}

	if (op->mexpr.base == X86_REG_EBP || op->mexpr.base == X86_REG_R13D ||
	    op->mexpr.base == X86_REG_RBP || op->mexpr.base == X86_REG_R13) {
	    switch (mod) {
	    case 0:
...
static int decode_modrm_reg(x86_dasm_context_t *x86_dctx, x86_operand_t *op,
                            uint8_t base_reg) {
...
static int decode_modrm_mod_rm(x86_dasm_context_t *x86_dctx, x86_operand_t *op,
	...
	*/
	...
	if (mod == 0 && rm == 6) {
	    ...
	    op->mexpr.disp_size =
	        (mod == 2 ? X86_DISP_16 : op->mexpr.disp_size);
	}
...
static int decode_opsize(x86_dasm_context_t *x86_dctx,
                         const x86_table_entry_t *te, int i) {
...
static int decode_optype(x86_dasm_context_t *x86_dctx,
                         const x86_table_entry_t *te, int i) {
    uint8_t j, x;
	...
	*/
	if (MODRM_MOD(get_modrm(x86_dctx)) != 3) {
	    dbg_err("the R/M field of the ModR/M byte may refer only "
	...
	*/
	...
	switch (x86_dctx->asize) {
	case X86_ASIZE_16BIT:
	    op->mexpr.base = X86_REG_SI;
	    break;
	case X86_ASIZE_32BIT:
	    op->mexpr.base = X86_REG_ESI;
	    break;
	case X86_ASIZE_64BIT:
	    op->mexpr.base = X86_REG_RSI;
	    break;
	...
	*/
	...
	switch (x86_dctx->asize) {
	case X86_ASIZE_16BIT:
	    op->mexpr.base = X86_REG_DI;
	    break;
	case X86_ASIZE_32BIT:
	    op->mexpr.base = X86_REG_EDI;
	    break;
	case X86_ASIZE_64BIT:
	    op->mexpr.base = X86_REG_RDI;
	    break;
...
static int decode_fix(x86_dasm_context_t *x86_dctx) {
	...
	if (op[0].type == X86_OPTYPE_REG && op[1].type == X86_OPTYPE_REG) {
	    if (op[0].reg >= X86_REG_CR0 && op[0].reg <= X86_REG_CR15) {
		op[0].reg = X86_REG_CR8;
	    } else if (op[1].reg >= X86_REG_CR0 && op[1].reg <= X86_REG_CR15) {
	...
	if (op[0].type == X86_OPTYPE_MEXPR && op[1].type == X86_OPTYPE_REG) {
    ...
    /* fix xrstor */
    if (x86_dctx->mnem == X86_MN_XRSTOR && x86_dctx->dmode == X86_DMODE_64BIT &&
        REX_W) {
    ...
    /* fix xsave */
    if (x86_dctx->mnem == X86_MN_XSAVE && x86_dctx->dmode == X86_DMODE_64BIT &&
        REX_W) {
    ...
    /* fix fxsave */
    if (x86_dctx->mnem == X86_MN_FXSAVE && x86_dctx->dmode == X86_DMODE_64BIT &&
        REX_W) {
    ...
    /* fix nop or pause */
    if (x86_dctx->mnem == X86_MN_XCHG && x86_dctx->pfx_p_rex == false &&
...
static int decode_operand(x86_dasm_context_t *x86_dctx,
                          const x86_table_entry_t *te, int i) {
...
static void decode_clear(x86_dasm_context_t *x86_dctx) {
    x86_operand_t *op = &x86_dctx->operands[0];
...
bool_t x86_resolve_rip(x86_dasm_context_t *x86_dctx, ulong_t i, uint64_t *out) {
    x86_operand_t *op = &x86_dctx->operands[i];
...
static uint64_t x86_resolve_reg(uint8_t reg, x86_regs_t *regs) {
    int i = 0;
	...
	i = reg - X86_REG_RAX;
    } else if (reg >= X86_REG_EAX && reg <= X86_REG_EIP) {
	mask = BIT_MASK(uint64_t, 32);

	/* compute register index */
	i = reg - X86_REG_EAX;
    } else if (reg >= X86_REG_AX && reg <= X86_REG_R15W) {
	mask = BIT_MASK(uint64_t, 16);

	/* compute register index */
	i = reg - X86_REG_AX;
    } else if (reg >= X86_REG_AL && reg <= X86_REG_R15L) {
...
uint64_t x86_resolve_op(x86_dasm_context_t *x86_dctx, x86_regs_t *regs,
                        ulong_t i) {
    uint64_t result;
	...
	if (op->mexpr.index != X86_REG_NONE) {
	    result +=
	        (x86_resolve_reg(op->mexpr.index, regs) * op->mexpr.scale);
	}
...
static void format_operand(x86_dasm_context_t *x86_dctx, int i) {
    char tmp[32], *fmt;
	...
	/* far jmp and call */
	if (x86_dctx->mnem == X86_MN_JMPF || x86_dctx->mnem == X86_MN_CALLF) {
	    strcat(op->str, "far ");
	}

	/* format segment prefix */
	if (op->size != X86_OPSIZE_NONE) {
	    sprintf(tmp, "%s ptr %s:", get_opsize_str(op->size),
	            regs_str[op->seg]);
	} else {
...
static int format_instruction(x86_dasm_context_t *x86_dctx,
                              const x86_table_entry_t *te) {
    char tmp[128];
	...
	strcat(tmp, "lock ");
    } else if (x86_dctx->pfx_p_rep && te->properties & PF_REP) {
	strcat(tmp, "rep ");
    } else if (x86_dctx->pfx_p_rep && te->properties & PF_REPE_REPNE) {
	strcat(tmp, "repe ");
    } else if (x86_dctx->pfx_p_repne && te->properties & PF_REPE_REPNE) {
	strcat(tmp, "repne ");
    }
	...
	char *cc_str[] = {/* SSE and AVX */
	                  "eq", "lt", "le", "unord", "neq", "nlt", "nle", "ord",

	                  /* AVX */
	                  "eq_uq", "nge", "ngt", "false", "neq_oq", "ge", "gt",
	                  "true", "eq_os", "lt_oq", "le_oq", "unord_s",
	                  "neq_us", "nlt_uq", "ord_s", "eq_us", "nge_uq",
	                  "ngt_uq", "false_os", "neq_os", "ge_oq", "gt_oq",
	                  "true_us"};

	char *xopcc_str[] = {"lt", "le",  "gt",    "ge",
	                     "eq", "neq", "false", "true"};
	...
	} else if (te->properties & PF_XOPCC) {
