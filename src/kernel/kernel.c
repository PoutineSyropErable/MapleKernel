...
void kernel_main(uint32_t mb2_info_addr, uint32_t magic,
                 uint32_t is_proper_multiboot_32) {
...
#define BIOS_FRAMEBUFFER_HACK
    ...
    kprintf("FB addr: %h %h, pitch=%u, w=%u, h=%u, bpp=%u, type=%u\n",
            ...
            grub_fb_info.pitch, grub_fb_info.width, grub_fb_info.height, bpp,
            grub_fb_info.type);
    ...
    volatile struct color_t *base_address =
        (struct color_t *)grub_fb_info.base_addr_low;
    ...
    uint16_t result = call_real_mode_function(
        add16_ref, width, height); // argc automatically calculated
