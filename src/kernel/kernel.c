...
void analyse_code(struct CodeAddressesToAnalyse code)
{
    ...
    terminal_writestring("\n");
    terminal_writestring(
        "The value of the code at call_real16_function_address: \n");
    for (int i = 0; i < 50; i++)
    {
...
void before() { terminal_writestring("Before the main execution\n"); }
...
void kernel_test()
{
    ...
    print_extern_address(
        "The address of stack16_start: ", get_stack16_start_address);
    print_extern_address(
        "The address of stack16_end: ", get_stack16_end_address);
    print_extern_address(
        "The address of args16_start: ", get_args16_start_address);
    print_extern_address("The address of args16_end: ", get_args16_end_address);

    print_extern_address("The address of the nice wrapper (>2MB): ",
        (address_getter_function_t *)get_call_realmode_func_with_args_address);
    [[maybe_unused]] int *pm32_to_pm16_address =
        print_extern_address("The address of pm32_to_pm16: ",
            (address_getter_function_t *)get_pm32_to_pm16_address);
    [[maybe_unused]] int *pm16_to_real16_address =
        print_extern_address("The address of pm16_to_real16: ",
            (address_getter_function_t *)get_pm16_to_real16_address);
    [[maybe_unused]] int *call_real16_function_address =
        print_extern_address("The address of call_real16_function_address: ",
            (address_getter_function_t *)get_call_real16_function_address);
    [[maybe_unused]] int *resume32_address =
        print_extern_address("The address of resume32: ",
            (address_getter_function_t *)get_resume32_start_address);
    [[maybe_unused]] int *resume32_end_address = print_extern_address(
        "The address of resume32_end: ", get_resume32_end_address);
...
// #define ANALIZE_CODE
    ...
    [[gnu::unused]] uint16_t result5 =
        call_real_mode_function(ret_5); // argc automatically calculated

    print_args16(&args16_start);
    kprintf("\n====ok====\n");

    uint16_t result = call_real_mode_function(
        add16_ref, 104, 201); // argc automatically calculated
    print_args16(&args16_start);
    terminal_write_uint("\nThe result of the real mode call is: ", result);

    uint16_t result2 = call_real_mode_function(
        complex_operation, 104, 201, 305, 43); // argc automatically calculated
    print_args16(&args16_start);
...
one_keyboard_one_mouse:
    ...
    enum ps2_device_super_type port1_super_type =
        get_device_super_type(port1_type);
    enum ps2_device_super_type port2_super_type =
        get_device_super_type(port2_type);
    ...
    assert(port1_super_type != port2_super_type,
        "It should be one keyboard, one mouse. Both can't be the same!\n");
    ...
    if (port1_super_type == PS2_DST_keyboard &&
        port2_super_type == PS2_DST_mouse)
    {
        keyboard_port = 1;
        mouse_port    = 2;
    ...
    else if (port1_super_type == PS2_DST_mouse &&
             port2_super_type == PS2_DST_keyboard)
    {
        mouse_port    = 1;
        keyboard_port = 2;
    ...
    kprintf("\nKeyboard type: |%s|, Mouse type: |%s|\n\n",
        ps2_device_type_to_string(keyboard_type),
        ps2_device_type_to_string(mouse_type));
...
one_port_only:
    ...
    enum ps2_device_super_type only_port_super_type =
        get_device_super_type(only_port_type);
...
two_keyboard:
    ...
    enum ps2_device_super_type keyboard1_super_type =
        get_device_super_type(keyboard1_type);
    enum ps2_device_super_type keyboard2_super_type =
        get_device_super_type(keyboard2_type);
    assert(keyboard1_super_type == keyboard2_super_type &&
               keyboard1_super_type == PS2_DST_keyboard,
    ...
    struct idt_fields_2_keyboard args_two_keyboard_value = {
        .keyboard1_port = 1, .keyboard2_port = 2};
...
two_mouse:
    ...
    enum ps2_device_super_type mouse1_super_type =
        get_device_super_type(mouse1_type);
    enum ps2_device_super_type mouse2_super_type =
        get_device_super_type(mouse2_type);
    assert(mouse1_super_type == mouse2_super_type &&
               mouse1_super_type == PS2_DST_keyboard,
        "Inconsistent super types. Should be two mouse. Not treated for now, "
        "assumed as a bug!\n");

    struct idt_init_ps2_fields args_two_mouse;
    struct idt_fields_2_mouse  args_two_mouse_value = {
         .mouse1_port = 1, .mouse2_port = 2};
...
void kernel_main(void)
{
    ...
    [[gnu::unused]] enum handle_ps2_setup_errors err_discard =
        handle_ps2_setup();
